            ...
            <form action="{{ route('faq.update', ['faq' => $faq->id]) }}" method="post"
                ...
                <div class="row">
                    ...
                    <div class="col-sm-12">
                        ...
                        <div class="form-group">
                            <label>Answer</label>
                            <textarea class="form-control" name="answer" id="" cols="10"
                        ...
                        <select class="form-select" name="doctor[]" id="" multiple>
                            @forelse($doctors as $doctor)
                                <option value="{{ $doctor->id }}"
                                        @if(in_array($doctor->id, $selected_doctors)) selected @endif>{{ $doctor->user->name }}</option>
                            @empty
                        ...
                        <select class="form-select" name="hospital[]" id="" multiple>
                            @forelse($hospitals as $hospital)
                                <option value="{{ $hospital->id }}"
                                        @if(in_array($hospital->id, $selected_hospitals)) selected @endif>{{ $hospital->name }}</option>
                            @empty
