            ...
            <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach ($queries as $data)
                        <tr>
                            <td>{{ $data->query_hash }} @if ($data->current_step == 1 || ($data->type == App\Models\Query::TYPE_MEDICAL_VISA && $data->current_step == 3))
...
@push('scripts')
    ...
    <script>
        async function edit(id) {
            let res = await fetch(route('designation.show', {
                'designation': id
            }));
            let data = await res.json();
            $form = $("#edit_designation-form input");
            $("#edit_designation-form").attr('action', route('designation.update', {
                'designation': id
            }));
